% Command
if ismac()
    decodecmd = 'cutest2matlab_osx';
else
    decodecmd = 'cutest2matlab';
end

% Extract files
files = dir('sif_data/*.sif');
files = files(1);
for file = files'
    [~, probname, ~] = fileparts(file.name);
    sif2mat(probname);
end
...
function [] = sif2mat(probname)
% Decode problem
fprintf('Decoding %s...', probname);
[status, cmdout] = system(sprintf('cutest2matlab_osx sif_data/%s', probname));
if status
    fprintf(cmdout);
...
mc = problem.m;         % Number of constraints (no bounds)
m = mc + n;             % Number of constraints (including bounds)
x0 = problem.x + 1;     % Initial guess of primal solution (used to evaluate gradients)
v0 = problem.v + 1;     % Initial guess of dual solution

% Objective
P = sparse(cutest_sphess(x0, v0));   % Hessian (TODO: Use cutest_sphess)
Px = cutest_hprod(x0);             % P * x
q = cutest_grad(x0) - Px;          % q
...
if norm(obj_cutest - obj_rewritten) > 1e-6
    error('Objective function decoding failed')
end
...
% [~, A] = cutest_lagjac(x0);   % Compute Jacobian
...
delete *.f *.o *.d *.SIF *.dylib *.mex* *.CNF *.67 fort*
end